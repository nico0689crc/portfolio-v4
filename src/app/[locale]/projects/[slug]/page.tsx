...
import { ArrowLeft, ExternalLink, Github, Users, Target, Lightbulb, Pencil, FlaskConical, Rocket, ChevronLeft, ChevronRight } from "lucide-react";
      ...
      <section className="section-padding pb-12">
        <div className="container mx-auto max-w-4xl">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mb-6">{c("overview")}</p>
            ...
            <div className="flex gap-4">
              {project.github && (
                <a href={project.github} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-foreground text-background text-sm font-medium hover:opacity-90 transition">
                  <Github className="w-4 h-4" /> {t("projects.code")}
                </a>
              )}
              <a href={project.demo} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-accent text-accent-foreground text-sm font-medium hover:opacity-90 transition">
      ...
      <section className="section-padding bg-muted/30">
        <div className="container mx-auto max-w-4xl">
          <motion.div {...fade} className="text-center mb-14">
            ...
            <p className="text-muted-foreground max-w-lg mx-auto">{t("case.processDesc")}</p>
            <div className="w-12 h-1 bg-accent mx-auto rounded-full mt-4" />
          </motion.div>

          <div className="space-y-12">
            {/* 1. Empatizar */}
            <PhaseCard icon={Users} phase={t("case.phase1.label")} title={t("case.phase1.title")}>
              <p>{c("empathize")}</p>
            </PhaseCard>

            {/* 2. Definir */}
            <PhaseCard icon={Target} phase={t("case.phase2.label")} title={t("case.phase2.title")}>
              <p>{c("define")}</p>
            </PhaseCard>

            {/* 3. Idear */}
            <PhaseCard icon={Lightbulb} phase={t("case.phase3.label")} title={t("case.phase3.title")}>
              <p>{c("ideate")}</p>
            </PhaseCard>

            {/* 4. Prototipar */}
            <PhaseCard icon={Pencil} phase={t("case.phase4.label")} title={t("case.phase4.title")}>
              <p>{c("prototype")}</p>
            </PhaseCard>

            {/* 5. Testear */}
            <PhaseCard icon={FlaskConical} phase={t("case.phase5.label")} title={t("case.phase5.title")}>
              <p>{c("test")}</p>
            </PhaseCard>
          </div>
        </div>
