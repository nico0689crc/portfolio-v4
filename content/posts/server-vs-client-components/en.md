...
seoTitle: "Server Components vs Client Components in Next.js: real-world criteria"
seoDescription: "When to use Server Components and when to use Client Components in Next.js App Router, with real examples from a portfolio in production and the reasoning behind each choice."
...
The question I get asked most about Next.js App Router isn't "how does it work" — the docs explain that reasonably well. It's "how do I decide, component by component, which is which?". That's the question the docs answer worst, because the answer depends on the project, not the technology.
...
## The real distinction isn't "server vs client"
...
Every Client component adds JavaScript the browser has to download, parse, and execute before the page can respond to a click. On a phone with a bad connection, that's the difference between a page that feels instant and one that feels stuck for two seconds "loading internally" even though it already looks complete.
