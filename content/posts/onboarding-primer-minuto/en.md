---
slug: the-first-minute-that-decides-if-someone-comes-back
...
A tutorial explains functionality. A well-designed onboarding demonstrates value. Those are different goals, and most products design the first one thinking they're doing the second.
...
## The case where this applies in practice

When I designed a new member's flow in GymSmartAccess, the path doesn't start with an explanation of the QR system. It starts with the member receiving the payment link — already with their name and the correct amount — and seeing confirmation that they can get into the gym immediately after paying. All the learning about how the QR works, where to see payment history, how to contact the gym, comes afterwards. Proof that the system solves what it needs to solve, first.
...
An onboarding doesn't exist so the person knows how to use the product. It exists so the person decides they want to learn how to use it. That decision gets made in the first minute, or it never gets made, and no complete tutorial makes up for it if that first minute proved nothing.
