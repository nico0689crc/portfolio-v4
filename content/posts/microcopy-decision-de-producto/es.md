---
slug: microcopy-es-una-decision-de-producto
...
imagePrompt: "Flat editorial vector illustration, modern tech-magazine spot art, geometric and diagrammatic. A single interface button repeated four times down a column, each version a slightly different width and weight. A small decision fork is drawn beside the column, resolving toward one of them. Composition: single focal cluster centred slightly left, wide empty margins, nothing important in the outer 8 percent. Palette: deep navy ground #0F172A, desaturated slate mid-tones #1E293B and #334155, off-white #F8FAFC for thin lines and highlights, and a single amber #F59E0B accent used only on the chosen button. Soft light from the top left, long low-contrast shadows, layered overlapping planes for depth. Fine film grain. No text, no letters, no numbers, no logos, no user interface chrome, no faces, no photorealism, no mesh gradients, no drop shadows on text. Aspect ratio 1.91:1, 1200x630."
---
...
## Los tres lugares donde más se nota

**Estados vacíos.** "No hay resultados" es información. "Todavía no armaste ningún proyecto — creá el primero para empezar" es una invitación. Un estado vacío bien escrito convierte el momento más deprimente de una interfaz —la pantalla en blanco— en el primer empujón hacia usar el producto.

**Mensajes de error.** Un buen mensaje de error contesta dos preguntas: qué pasó y qué hago ahora. "El email ya está registrado" contesta la primera y deja a la persona sola con la segunda. "Ese email ya tiene una cuenta — ¿querés iniciar sesión en vez de crear una nueva?" contesta las dos y además ofrece la salida.

**Confirmaciones de acción irreversible.** "¿Estás seguro?" no le da a nadie información nueva para decidir. "Esto va a eliminar los 34 socios cargados y no se puede deshacer" sí. La diferencia entre las dos frases es la diferencia entre una confirmación que se aprieta sin leer y una que realmente hace pensar dos segundos antes.
...
En el rediseño de Mexx, uno de los hallazgos fue que los productos digitales llegaban al checkout con costos de envío físico sin explicación. Parte de la solución no fue solo técnica —desglosar el costo antes— sino textual: la etiqueta "Producto digital — sin envío" en la tarjeta del producto, antes de que la persona llegara siquiera al carrito.

Esa etiqueta es dos palabras y un guion. Y forma parte de la razón por la que la tarea de transparencia de costos bajó su tasa de abandono en el test de usabilidad posterior. El texto no fue decoración de la solución. Fue la solución, en parte.

## Cómo lo integro en el proceso
