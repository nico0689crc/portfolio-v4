...
seoTitle: "De Figma a código: cómo llevar un diseño a producción sin perder fidelidad"
seoDescription: "El flujo que uso para pasar un diseño de Figma a componentes React en producción: qué se decide en el diseño, qué se decide en el código, y por qué el orden importa."
...
Cuando el diseñador y el desarrollador son la misma persona, desaparece un problema clásico —la pelea por teléfono descompuesto entre dos roles— y aparece uno nuevo, menos hablado: la tentación de saltarte pasos porque "ya sé lo que quiero", y descubrir en el código que no lo sabías tan bien como creías.

Este es el flujo que uso para que el paso de Figma a producción no pierda fidelidad, ni siquiera cuando nadie más lo está revisando.

## La regla que ordena todo

**Si no se puede implementar sin adivinar, no está diseñado. Está dibujado.**

Un archivo de Figma bonito no es un design system. Es una ilustración de cómo se vería un design system si existiera. La diferencia se nota en un solo lugar: qué tan fácil es que un desarrollador —en mi caso, yo mismo tres días después— tome una decisión sin tener que abrir el archivo original a adivinar un valor.

## Paso 1: tokens antes que pantallas
...
Esto invierte el orden natural. La tentación es diseñar la pantalla y "sacar" los tokens después, mirando lo que quedó. Hacerlo al revés obliga a decidir el sistema antes de tener una pantalla que lo justifique, y ese sistema es exactamente lo que se convierte en variables de Tailwind sin traducción de por medio.

## Paso 2: componentes con estados, no pantallas con casos

Diseño un botón, no cuarenta pantallas que cada una tiene un botón un poco distinto. Y el botón se diseña con sus estados: default, hover, disabled, loading. No es un capricho de prolijidad — es la diferencia entre un componente de React con props claras y un desarrollador que tiene que inventar el estado `disabled` porque nadie lo diseñó y "algo hay que poner".

Cuando el diseño ya trae los cuatro estados, el componente en código sale con cuatro variantes previstas. Cuando no los trae, el desarrollador improvisa, y esa improvisación es exactamente el punto donde la interfaz final deja de parecerse al diseño.

## Paso 3: el prototipo interactivo, no la lámina estática

Uso Figma para armar el flujo completo navegable, no capturas sueltas. La razón es que un flujo navegable expone problemas que una lámina no muestra: una transición que no tiene sentido, un estado de carga que nadie dibujó, un camino de vuelta que no existe.

En el caso de Mexx, seis animaciones de interfaz quedaron documentadas explícitamente —el slider del hero, el modal de autenticación, el desglose de costos, el indicador de guardado, los estados de carga, las transiciones de checkout. Ninguna se improvisó en el código porque ninguna llegó al código sin estar antes decidida en el diseño.
