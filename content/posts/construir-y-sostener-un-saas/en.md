...
seoTitle: "Building and running a SaaS: what I learned with paying customers"
seoDescription: "What changes between building a product and running it in production with real customers: support, technical debt, and the decisions I made building GymSmartAccess."
...
Building a product and running it are two different jobs, and almost nobody tells you that before you're stuck doing the second one. Building [GymSmartAccess](/en/projects/gym-smart-access) — my SaaS platform for independent gyms, with automated billing and QR-based access control — was maybe 30% of the real work. The other 70% started the day the first gym began paying.
...
I learned to design so a missed webhook can be reconstructed afterwards, by checking the real state in Mercado Pago instead of blindly trusting that every event arrived. It's more upfront work, and it's the difference between a system that self-corrects and one that needs me to notice and fix it by hand.
...
**Technical debt gets charged with interest, not all at once.** A rushed decision in month 2 — a table missing the right index, a validation "I'll add later" — doesn't show up until month 8, when there's ten times more data and the problem that used to be invisible turns into a query that takes three seconds.

## The decisions I made thinking about running it, not building it

I chose managed infrastructure — Supabase, Vercel — over my own servers, precisely because I'm a single developer keeping this running. Your own server is cheaper on the monthly bill and more expensive in the hours you spend keeping it alive. That math changes completely when the person who answers at 2am is you and nobody else.

I chose to automate the webhook flows with Make.com instead of writing every integration from scratch myself, because the goal wasn't to prove I could write the code — it was to minimize the surface of things that could break silently while I slept.

I chose a simple front-desk monitor, with clear visual states, over a panel with more data, because in production I understood something you can't see in development: the person watching that screen at a gym's front desk has no time to interpret a dashboard. They need to know in two seconds whether the member gets in or not.

## What a case study usually doesn't show

Most case studies end at launch, with a nice screenshot and a list of results. GymSmartAccess's real results aren't only the numbers I show in the portfolio — eliminating delinquency from manual billing, zero hardware cost, full payment automation. They're also the things I had to fix after it was already in production: a query that started slowing down once we went from dozens to hundreds of members, a case where two Mercado Pago webhooks arrived in the wrong order and duplicated a state, a monitor screen that read poorly under direct sunlight at a gym's entrance.
