---
slug: personas-de-ux-con-datos-reales
...
imagePrompt: "Flat editorial vector illustration, modern tech-magazine spot art, geometric and diagrammatic. A persona card in which the portrait area is replaced by a small behaviour chart. Three short evidence lines run from the card back to a stack of survey responses behind it. Composition: single focal cluster centred slightly left, wide empty margins, nothing important in the outer 8 percent. Palette: deep navy ground #0F172A, desaturated slate mid-tones #1E293B and #334155, off-white #F8FAFC for thin lines and highlights, and a single amber #F59E0B accent used only on the behaviour chart standing in for the portrait. Soft light from the top left, long low-contrast shadows, layered overlapping planes for depth. Fine film grain. No text, no letters, no numbers, no logos, no user interface chrome, no faces, no photorealism, no mesh gradients, no drop shadows on text. Aspect ratio 1.91:1, 1200x630."
---

Las personas de UX tienen mala fama, y es una fama merecida. La mayoría son un nombre inventado, una foto de banco de imágenes sonriendo, y tres datos demográficos que no cambian ninguna decisión de diseño. Se hacen, se imprimen, se pegan en una pared, y nadie las vuelve a mirar.

El problema no es la herramienta. Es que casi siempre se construye al revés.

## La pregunta que decide si sirve o no

Antes de armar una persona, me hago una pregunta: **¿qué decisión de diseño va a cambiar esto?**
...
Después de construir a Lucas y Sofía, volví a mirar la lista de funcionalidades que estábamos por priorizar. La recuperación de carrito por magic link — que estaba en el medio de la lista — subió de prioridad porque es exactamente lo que Sofía necesita: volver a un carrito que armó en otro dispositivo sin tener que buscar el producto de nuevo.

Ese es el momento que separa una persona útil de una decorativa: cuando cambia el orden de una lista, no cuando queda linda en una diapositiva.

## Lo que dejo afuera a propósito

No pongo nombre completo, ni ciudad de nacimiento, ni hobbies inventados. Cada dato extra que no salió de evidencia real es una oportunidad de que alguien en el equipo diseñe para el dato inventado en vez de para el patrón real. Si Lucas "vive en Palermo y le gusta el mate", esa frase no ayuda a decidir nada — solo hace que la persona se sienta más real sin serlo.

Cuantos menos datos decorativos, más fácil es ver cuáles importan.
