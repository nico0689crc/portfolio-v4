-- Textos de SEO reorientados a reclutadores, dentro y fuera del país.
--
-- Todo lo que sigue es contenido, no estructura: se puede editar después desde
-- /admin/seo sin tocar código. Está acá para que el cambio quede versionado y
-- revisable en lugar de aplicado a mano contra la base.
--
-- Tres criterios detrás de cada texto:
--
-- 1. El título entra en la SERP. El template agrega " | Nicolás Ariel
--    Fernández" (25 caracteres) a todas las rutas salvo la portada, que emite
--    el suyo completo. Ninguno de los de abajo pasa los 60 con el sufijo.
--
-- 2. Las descripciones van entre 120 y 155. Más corto desperdicia el espacio,
--    más largo se corta a mitad de frase.
--
-- 3. Lo que un reclutador filtra primero no es el stack: es dónde estás, en qué
--    horario y en qué idioma trabajás. Por eso "remoto", "Europa", "EE. UU." e
--    "inglés" aparecen en las descripciones y no enterrados en la página.
