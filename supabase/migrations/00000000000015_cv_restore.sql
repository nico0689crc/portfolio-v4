-- Restaura el CV que el seed borró.
--
-- La migración 12 cargó estas filas y una corrida posterior de
-- `scripts/seed-content.mjs` las eliminó: `resolveOrdered()` borra la tabla
-- entera cuando el conteo de filas no coincide con el del archivo estático, y
-- reinserta desde ahí. Con siete experiencias en la base contra cinco en
-- `cvData.ts`, se llevó puestas las dos nuevas y las seis certificaciones.
--
-- Esta versión es idempotente: cada inserción comprueba que la fila no exista,
-- así volver a correrla no duplica nada. El seed queda con un guard en el mismo
-- commit para que no pueda repetirse.
...
-- ------------------------------------------------------------------ orden
-- Sólo corre si las nuevas todavía no están, para no desplazar dos veces.
update public.experiences set sort_order = sort_order + 2
where not exists (select 1 from public.experiences where organization = 'EVOLVERE');
...
with nueva as (
  ...
  select
    ...
    '2026-02-01', null, 1
  where not exists (select 1 from public.experiences where organization = 'GymSmartAccess')
  returning id
)
insert into public.experience_translations (experience_id, locale, role, location, date_label, description)
select nueva.id, t.locale, t.role, t.location, t.date_label, t.description
...
) as nuevas(issuer, year, url, sort_order)
where not exists (
  select 1 from public.certifications c where c.url = nuevas.url
);
